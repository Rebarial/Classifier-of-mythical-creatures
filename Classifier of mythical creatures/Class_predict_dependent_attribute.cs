...
{
    public partial class Class_predict_dependent_attribute : Form
    {
        ...
        public Class_predict_dependent_attribute(string idPAtt, List<(string, string, string, string)> dac)
        {
            dependentAttInClass = dac;
            pattId = idPAtt;
        ...
        private void InitializeData()
        {
            ...
            Button buttonNew1;
            int position = 0;
            foreach (var a in dependentAttInClass)
            {
                labelNew = new Label();
            ...
            dependentAttNotInClass = model.ReadDependentAttributeNotInClassByArrow(dependentAttInClass, pattId);
        ...
        private int getElemFromId(string id)
        {
            int c = 0;
            foreach (var a in dependentAttInClass)
            {
        ...
        private void DelDependentAttToClass(object sender, EventArgs e)
        {
            ...
            dependentAttInClass.RemoveAt(getElemFromId(a.Name));
            InitializeData();
        }
        private void EditDependentAttToClass(object sender, EventArgs e)
        {
            ...
            if (s.Item2 != "")
            {
                if (s.Item1 == "1")
                {
                    if (a.Text != "")
                    {
                        dependentAttInClass[getElemFromId(a.Name)] = (dependentAttInClass[getElemFromId(a.Name)].Item1, dependentAttInClass[getElemFromId(a.Name)].Item2, textBox.Text, pattId);
                        //d.Item3 = textBox.Text;
        ...
        private void AddDependentAttToClass(object sender, EventArgs e)
        {
            if (comboBoxNew.Text != "")
            {
                ...
                {
                    case "1":
                        dependentAttInClass.Add((attId, comboBoxNew.Text, "Да", pattId));
                        //model.AddDependentAttributeToClass(id, attId, "Да");
                        break;
                    case "2":
                        //model.AddDependentAttributeToClass(id, attId, a.Item2.Split(',').ToList()[0]);
                        dependentAttInClass.Add((attId, comboBoxNew.Text, a.Item2.Split(',').ToList()[0], pattId));
                        break;
                    case "3":
                        //model.AddDependentAttributeToClass(id, attId, a.Item2.Substring(0, a.Item2.IndexOf(':')));
                        dependentAttInClass.Add((attId, comboBoxNew.Text, a.Item2.Substring(0, a.Item2.IndexOf(':')), pattId));
                        break;
                    default:
                        break;
